<template>
  <div :id="'rockiot-wrapper-' + serial" class="rockiot-wrapper" :ref="'rockiot-ui-' + serial" @click="showControl = !showControl">
    ...
    <div v-if="type != 'dashboard'" :class="classe + ' rockiot-ui-' + type" :style="'background:' + chartBackground + ';'">
      <rockiot-ui-control v-bind="_props" :showControl="showControl" @startTest="clickGauge = !clickGauge" @setting="displayAttributes = !displayAttributes" @connect="$emit('connect')" @custom="emitAction" @fullscreen="((fullscreen = !fullscreen), $emit('fullscreen'))"></rockiot-ui-control>

      <component v-if="type === 'chart'" :is="chartComponent" :component="chartComponent" :type="areaChart" v-bind="_props" :value="updatedValue" />

      <component v-if="type === 'gauge'" :is="gaugeComponent" :component="gaugeComponent" :setting="settingsChanged" @updated="settingsChanged = false" v-bind="elementAttributes" :value="updatedValue" />

      <rockiot-number-box v-if="type === 'number-box'" v-bind="elementAttributes" :value="updatedValue" />
    </div>
  </div>
</template>
    ...
    clicked() {
      ...
      if (!this.timer[this.serial]) {
        this.timer[this.serial] = setInterval(() => {
          self.updatedValue = Math.random() * (parseInt(self.max) - parseInt(self.min)) + 1 + parseInt(self.min)
        }, 1000)
