<template>
  <div style="padding-top: 1rem">
    <svg :style="$attrs.svgStyle" :class="'typeRange rockiot-gauge rockiot-gauge-' + $attrs.size" :height="svgheight" :width="svgwidth" :view-box="'0 0 ' + svgwidth + ' ' + svgheight" :ref="$attrs.serial" :id="$attrs.serial">
      ...
      <rect class="outline" :id="'outline-' + $attrs.serial" :style="outlineStyle" :x="offsetX" :width="svgwidth - offsetX * 2" :height="barHeight" :y="offsetY - barHeight" />

      <rect class="fill" :style="fillStyle" :id="'fill-' + $attrs.serial" :x="offsetX" :width="pos" :height="barHeight" :y="offsetY - barHeight"></rect>

      <rect v-if="$attrs.needle === '1'" :id="'needle-' + $attrs.serial" class="needle" :style="animate('x')" width="1" :x="this.pos + this.offsetX" :y="offsetY - barHeight - 5" :height="barHeight + 10" :fill="$attrs.needleColor" />

      <template v-for="(zone, i) in limitzones">
        <rect v-if="zone" :key="'zone-' + i" class="rockiot-zones" :id="'zones-' + i + '-' + $attrs.serial" :fill="zone" :x="offsetX + zoneWidth * i" :width="zoneWidth" height="3" :y="offsetY + 1" />
      </template>
    </svg>
    <div class="rockiot-gauge-linear-vertical-output">
      <div class="rockiot-gauge-linear-vertical-name">{{ this.$attrs.name }} {{ this.$attrs.units }}</div>
      <div :class="'rockiot-gauge-value rockiot-gauge-' + $attrs.variation + '-' + $attrs.orientation + '-value'">
        <rockiot-animated-number :ref="'num_' + this.$attrs.serial" :precision="$attrs.precision" :duration="$attrs.animation" :from="oldValue" :to="$attrs.value" @end="oldValue = $attrs.value" />
      </div>
    </div>
  </div>
</template>
