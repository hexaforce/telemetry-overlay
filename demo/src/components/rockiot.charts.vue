<template>
  <div :class="'rockiot-chartist rockiot-chartist-' + $attrs.serial" :style="'background:' + $attrs.chartBackground"></div>
</template>
  ...
  methods: {
    ...
    createChart() {
      ...
      if (this.$attrs.variation != 'bar') {
        this.chart = new Rockiotchart.Line(
          '.rockiot-chartist-' + this.$attrs.serial,
          {
      ...
      if (this.$attrs.variation === 'bar') {
        this.chart = new Rockiotchart.Bar(
          '.rockiot-chartist-' + this.$attrs.serial,
          {
            labels: self.chartData.labels,
            series: [self.chartData.data],
