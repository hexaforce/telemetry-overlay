<template>
  <div>
    <svg :style="$attrs.svgStyle" :class="' rockiot-linear-vertical rockiot-gauge rockiot-gauge-' + $attrs.size" :height="svgheight" :width="svgwidth" :view-box="'0 0 ' + svgwidth + ' ' + svgheight" :ref="$attrs.serial" :id="$attrs.serial">
      ...
      <rect class="rockiot-outline" :id="'outline-' + $attrs.serial" :style="fillStyle" :x="offsetX" :height="svgheight - offsetY * 2" :width="barHeight" :y="offsetY"></rect>
      <rect class="rockiot-fill" :style="outlineStyle" :id="'fill-' + $attrs.serial" :x="offsetX" :height="pos" :width="barHeight" :y="offsetY"></rect>
      <rect :id="'needle-' + $attrs.serial" class="rockiot-needle" :style="animate('y')" height="1" :y="pos + offsetY" :x="offsetX" :width="barHeight" :fill="$attrs.needleColor" />

      <template v-for="(zone, i) in limitzones">
        <rect :key="'zone-' + i" class="rockiot-zones" :id="'zones-' + i + '-' + $attrs.serial" :fill="zone" :x="offsetX - 6" :height="zoneHeight" :width="5" :y="offsetY + zoneHeight * i" />
      </template>
    </svg>
